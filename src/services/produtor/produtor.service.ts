...
export class ProdutorService {
  ...
  async update(id: number, produtorUpdated: ProdutorDto): Promise<Produtor> {
  ...
  async retrieveStatistics(): Promise<EstatisticasDto> {
    let querysResult = await Promise.all([
      ...
      await this.produtorRepository.query(
        'select sum(p."areaTotal") from produtor p',
      ),
      await this.produtorRepository.query(
        'select p.estado as estado, count(p.id) as total from produtor p group by p.estado',
      ),
      await this.produtorRepository.query(
        'select c.nome as cultura, count(p.id) as total from produtor p join produtor_culturas_cultura pc on p.id = pc."produtorId" join cultura c on c.id = pc."culturaId" group by c.nome',
      ),
      await this.produtorRepository.query(
        'select sum(p."areaAgricultavel") from produtor p',
      ),
      await this.produtorRepository.query(
        'select sum(p."areaDeVegetacao") from produtor p',
      ),
    ]);
    ...
    let response: EstatisticasDto = {
      ...
      estatisticasEstado: querysResult[2].map((it) => {
        return { estado: it.estado, total: +it.total };
      }), // map para converter para number
      estatisticasCultura: querysResult[3].map((it) => {
        return { cultura: it.cultura, total: +it.total };
      }), // map para converter para number
  ...
  private validateProdutor(produtor: ProdutorDto) {
    ...
    let documento =
      produtor.documento && produtor.documento.replace(/[^\d]+/g, '');
    ...
    if (
      ...
      if (
        produtor.areaAgricultavel + produtor.areaDeVegetacao >
        produtor.areaTotal
      ) {
        throw new BadRequestException(
          'A soma de área agrícultável e vegetação está superior a área total da propriedade.',
        );
      }
    } else {
      throw new BadRequestException(
        'Dados de area da propriedade não informados.',
      );
    }
  }
